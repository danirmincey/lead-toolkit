...
(function () {
  ...
  function mount(container) {
    var state = {
      ...
      fontFamily: 'Candara',
      showYAxis: true,            // feedback: frequency axis on by default
      showMeanText: true
    };
    ...
    container.innerHTML = '' +
      ...
      '        <div class="clusterblock" id="me-clusterblock" style="display:none">' +
      ...
      '          <select id="me-font"><option value="Candara" selected>Candara</option><option value="Corbel">Corbel</option><option value="Arial">Arial</option><option value="Georgia">Georgia</option><option value="Calibri">Calibri</option></select></label>' +
      '        <label class="check"><input type="checkbox" id="me-yaxis" checked> Show frequency y axis</label>' +
      '        <label class="check"><input type="checkbox" id="me-meantext" checked> Show mean text</label>' +
    ...
    function render() {
      ...
      if (!data.length) {
        ...
        var em = $('me-empty');
        em.textContent = (state.view === 'dist' && state.clusterCol >= 0 &&
          state.includeValues !== null && state.includeValues.size === 0)
          ? 'tick your cluster(s) above to continue' : 'output displayed HERE';
        em.style.display = '';
        $('me-png').disabled = true;
        $('me-ppt').disabled = true;
        $('me-copy').disabled = true;
      ...
      var mR = 0.03 * W, mT = 0.10 * H, mB = 0.16 * H;
      ...
      // mean text over the score distribution (weighted when counts were edited)
      if (state.view === 'dist' && state.showMeanText) {
        var wn = 0, wsum = 0;
        data.forEach(function (d) {
          var lv = parseFloat(d[0]);
          if (isFinite(lv) && isFinite(d[1]) && d[1] > 0) { wn += d[1]; wsum += lv * d[1]; }
        });
        if (wn > 0) {
          var wm = wsum / wn, vsum = 0;
          data.forEach(function (d) {
            var lv = parseFloat(d[0]);
            if (isFinite(lv) && isFinite(d[1]) && d[1] > 0) vsum += d[1] * (lv - wm) * (lv - wm);
          });
          var wsd = wn > 1 ? Math.sqrt(vsum / (wn - 1)) : NaN;
          var meanPx = Math.round(labPx * 1.6);
          ctx.fillStyle = '#2E74B5';
          ctx.font = '700 ' + meanPx + 'px ' + fBody;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'top';
          ctx.fillText('Mean = ' + wm.toFixed(2), W / 2, 0.015 * H);
          if (isFinite(wsd)) ctx.fillText('(SD = ' + wsd.toFixed(2) + ')', W / 2, 0.015 * H + meanPx * 1.15);
        }
      }
    ...
    $('me-font').addEventListener('change', function (e) { state.fontFamily = e.target.value; scheduleRender(); });
    $('me-yaxis').addEventListener('change', function (e) { state.showYAxis = e.target.checked; scheduleRender(); });
    $('me-meantext').addEventListener('change', function (e) { state.showMeanText = e.target.checked; scheduleRender(); });
